...
choose_stats <-
  box(title = "Display stat", width = 6, background = "black",
      ...
      checkboxInput("show_ci", "Conf. interval on mean:", value = FALSE) %>%
        tighten(bottom = -10),
      checkboxInput("show_pred_interval", "Pred. interval", value = FALSE) %>%
        tighten(bottom = -10),
    ...
    construct_plot <- reactive({
      ...
      if (input$var_y %in% names(plot_data)  && input$var_y %in% names(plot_data))  {
        ...
        For_sd_ruler <-
          ...
          select(- mean, - sd,  - center)
        For_sd_ruler_labels <- For_sd_ruler %>%
          tidyr::gather(key = label, value = vertical, `pos_sd`, neg_sd, pos_2sd, neg_2sd)
        ...
        if (input$show_sd) {
          ...
          P <- P %>% gf_errorbar(this_formula, data = For_sd_ruler, color = "black", width = 0.1)
          this_formula <- as.formula(glue::glue("pos_2sd + neg_2sd ~ {input$var_x}"))
          P <- P %>% gf_errorbar(this_formula, data = For_sd_ruler, color = "black", width = 0.2)
        ...
        if (input$var_x == "1")
          P <- P %>% gf_lims(x = c(0,2)) %>% gf_labs(x = "")  %>% gf_theme(no_x_axis)
